...
Parser::Parser(const std::string &input_file) {
    ...
    while (std::getline(file, line)) {
        ...
        if (!trimed_line.empty() && !trimed_line.starts_with("//")) {
            ...
            code_lines.push_back(trim(trimed_line));
        }
    }
}
...
std::string Parser::advance() {
    ...
    if (current_line.starts_with("push")) {
        ...
        arg1_ = current_line;
        arg2_ = "";
    } else if (current_line.starts_with("goto")) {
        command_type_ = CommandType::C_GOTO;
        auto tokens = split(current_line, " \t");
        arg1_ = tokens[1];
        arg2_ = "";
    } else if (current_line.starts_with("if-goto")) {
        command_type_ = CommandType::C_IF;
        auto tokens = split(current_line, " \t");
        arg1_ = tokens[1];
        arg2_ = "";
    } else if (current_line.starts_with("label")) {
        command_type_ = CommandType::C_LABEL;
        auto tokens = split(current_line, " \t");
        arg1_ = tokens[1];
        arg2_ = "";
    } else if (current_line.starts_with("function")) {
        command_type_ = CommandType::C_FUNCTION;
        auto tokens = split(current_line, " \t");
        arg1_ = tokens[1];
        arg2_ = tokens[2];
    } else if (current_line.starts_with("call")) {
        command_type_ = CommandType::C_CALL;
        auto tokens = split(current_line, " \t");
        arg1_ = tokens[1];
        arg2_ = tokens[2];
    } else if (current_line.starts_with("return")) {
        command_type_ = CommandType::C_RETURN;
        auto tokens = split(current_line, " \t");
        arg1_ = "";
        arg2_ = "";
    } else {
        throw std::invalid_argument("<Parser> Invalid command: [" +
                                    current_line + "]");
    }
